...
__global__ void kernel( unsigned char *ptr) {
	...
	int index = blockIdx.x + threadIdx.x * DIM;
	//Declear the x position of the graph
	int temp_x = index % DIM;
	//Declear the y position of the graph
	int temp_y = index / DIM;

	float jx = 1.5*(float)(DIM / 2 - temp_x) / (DIM / 2);
	float jy = 1.5*(float)(DIM / 2 - temp_y) / (DIM / 2);
...
void julia_set_gpu() {
	...
	kernel<<<no_block,no_thread>>>(dev_bitmap);
